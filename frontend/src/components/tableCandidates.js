...
function TableCandidates(){
    ...
    function handleChangeApply(){
        let chosen = chosenColumns;
        fetch("http://localhost:3001/all")
            .then((res) => res.json())
            .then((data) => setCandidates(data));
        var orderedColumns = [];
        for(var k=0; k<columnsAll.length; k++){
            if(chosen.includes(columnsAll[k])){
    ...
    return (
        <div className="p-5">
            ...
            <header>
                <h1 className="text-4xl font-bold text-[#3079FF] my-4 mx-10">Candidates</h1>
            </header>
            <main className="flex flex-col mx-10">
                <div className="flex flex-col mb-6">
                    ...
                    <div className="grid grid-cols-6 gap-5 my-4 mx-8 p-4 rounded-lg">{columnsAll.map(col => <div key={col} className="flex flex-row"><input id={col} type="checkbox" value className="mr-2 checked:bg-[#3079FF]" onChange={() => handleCheckboxSpecific(col)}></input><label htmlFor={col} className="capitalize">{col.includes('_') ? col.replaceAll('_', ' ') : col}</label></div>)}</div>
                </div>
                <div className="flex flex-row mb-6 justify-end">
                    <div className="mt-2 mx-12">
                        <input id="checkbox_all" type="checkbox" value className="mr-2 checked:bg-[#3079FF]" onChange={() => handleCheckboxAll()}></input>
                        <label htmlFor="checkbox_all">Select all</label>
                    </div>
                    <button onClick={()=>clearAllCheckbox()} className="border border-[#3079FF] rounded-lg py-2 px-3 text-[#3079FF] float-right mr-4">Clear all</button>
                    <button type="submit" onClick={()=>handleChangeApply()} className="border border-[#3079FF] bg-[#3079FF] rounded-lg py-2 px-3 text-white float-right">Apply</button>
                </div> 
                {chosenColumns.length !== 0 ? <div className="overflow-auto border-4 border-[#3079FF] rounded-lg">
                    <table className="border-collapse border border-[#3079FF] rounded-lg w-full">
                        <thead><tr>{chosenColumns.map(col => <th key={col} className="border border-[#3079FF] capitalize break-words p-5">{col.includes('_') ? col.replaceAll('_', ' ') : col}</th>)}<th>Condition</th></tr></thead>
                        <tbody>{candidates.map(c => <tr key={c.id}>{Object.keys(c).map(k => chosenColumns.includes(k) ? <td key={k} className="border border-[#3079FF] break-words text-center p-3">{((k === "cv_zonajobs" && c[k]) || (k === "cv_bumeran" && c[k])) ? <a href={c[k]} className="text-[#3079FF]" target="_blank" rel="noopener noreferrer">Check CV</a> : k === "date" ? c[k].substring(0,11) : c[k]}</td> : '')}<td className="border border-[#3079FF] p-3"><div className="flex flex-row justify-center">{c.reason ? <p className="text-red-500 pr-2">Rejected</p> : <p className="text-green-500 pr-2">Approved</p>}<button onClick={()=>{setModalView(m => !m); setCandidateToChange(c)}}><svg viewBox="0 0 24 24" width="20px" height="20px" className="" fill="none" xmlns="http://www.w3.org/2000/svg"> <path fillRule="evenodd" clipRule="evenodd" d="M21.1213 2.70705C19.9497 1.53548 18.0503 1.53547 16.8787 2.70705L15.1989 4.38685L7.29289 12.2928C7.16473 12.421 7.07382 12.5816 7.02986 12.7574L6.02986 16.7574C5.94466 17.0982 6.04451 17.4587 6.29289 17.707C6.54127 17.9554 6.90176 18.0553 7.24254 17.9701L11.2425 16.9701C11.4184 16.9261 11.5789 16.8352 11.7071 16.707L19.5556 8.85857L21.2929 7.12126C22.4645 5.94969 22.4645 4.05019 21.2929 2.87862L21.1213 2.70705ZM18.2929 4.12126C18.6834 3.73074 19.3166 3.73074 19.7071 4.12126L19.8787 4.29283C20.2692 4.68336 20.2692 5.31653 19.8787 5.70705L18.8622 6.72357L17.3068 5.10738L18.2929 4.12126ZM15.8923 6.52185L17.4477 8.13804L10.4888 15.097L8.37437 15.6256L8.90296 13.5112L15.8923 6.52185ZM4 7.99994C4 7.44766 4.44772 6.99994 5 6.99994H10C10.5523 6.99994 11 6.55223 11 5.99994C11 5.44766 10.5523 4.99994 10 4.99994H5C3.34315 4.99994 2 6.34309 2 7.99994V18.9999C2 20.6568 3.34315 21.9999 5 21.9999H16C17.6569 21.9999 19 20.6568 19 18.9999V13.9999C19 13.4477 18.5523 12.9999 18 12.9999C17.4477 12.9999 17 13.4477 17 13.9999V18.9999C17 19.5522 16.5523 19.9999 16 19.9999H5C4.44772 19.9999 4 19.5522 4 18.9999V7.99994Z" fill="#3079FF"></path></svg></button></div></td></tr>)}</tbody>
                    </table>
                </div> :
                ""}
            </main>
